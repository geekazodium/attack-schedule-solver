...
impl EnemyTrack{
    ...
    pub fn can_meet_request(&self, request_vec: &Vec<u64>, request_offset: usize, claim_end: u64) -> Vec<&Attack>{
        if let Some(request_frame) = request_vec.get(request_offset).map(|u| *u){
            self.attacks
                .iter()
                .filter_map(|attack| 
                    attack.get_start_frame(request_frame)
                    .map(|start_frame| (attack,start_frame)))
                .filter(|pair| {
                        dbg!(&pair.0);
                        let mut tmp_iter = request_vec[(request_offset + 1)..].iter();
                        dbg!(&tmp_iter);
                        let mut active_frames_iter = pair.0.get_active_frames(pair.1);
                        active_frames_iter.next();
                        for active in active_frames_iter{
                            dbg!(active);
                            if active >= claim_end{
                                return true;
                            }
...
mod enemy_track_tests{
    ...
    fn fail_meet_request_test(){
        ...
        assert!(mock_track.can_meet_request(&mut vec![19, 28], 0, 100).is_empty());
    }
}